...
class AdminEditBoat extends StatefulWidget {
  String boatname, price, quantity, imgUrl, boatid, package;
  bool bday = false, bnight = false;

  AdminEditBoat(this.boatname, this.price, this.quantity, this.imgUrl,
      this.boatid, this.package, this.bday, this.bnight);
  @override
  State<AdminEditBoat> createState() => _AdminEditBoatState(
      boatname, price, quantity, imgUrl, boatid, package, bday, bnight);
}

class _AdminEditBoatState extends State<AdminEditBoat> {
  String boatname, price, quantity, imgUrl, boatid, package;
  bool _day, _night;
  ...
  _AdminEditBoatState(this.boatname, this.price, this.quantity, this.imgUrl,
      this.boatid, this.package, this._day, this._night);

  FirebaseStorage storage = FirebaseStorage.instance;

  @override
  initState() {
    _conboatname.text = boatname;
    _conboatquantity.text = quantity;
    _conboatprice.text = price;
  ...
  addBoat() async {
    ...
    if (bname.isEmpty ||
      ...
      FirebaseFirestore.instance.collection('Boats').doc(boatid).set({
        ...
        if (!img) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text("Boat Edited Successfully!"),
          ));
          Navigator.of(context).pushAndRemoveUntil(
              MaterialPageRoute(
                  builder: (_) => AdminBoatDetails(boatid, bname, price, imgUrl,
                      quantity, service, package)),
              (Route<dynamic> route) => false);
        } else {
          try {
            await storage.ref("Boats/" + boatid + "/primary.jpeg").putFile(
                imageFile,
      ...
      }).catchError((error) => print("Failed to Edit Boat: $error"));
    }
  }
